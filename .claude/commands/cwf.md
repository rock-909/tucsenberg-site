...
**输出**：`docs/superpowers/workflows/cwf/{page}/v{N}.md` + 可选 i18n JSON。

Starter 只保留 CWF 这套 workflow，不携带旧项目已经跑出来的 CWF 文案产物。不要恢复或复用旧页面定稿、旧研究上下文或旧 proof materials。新项目如果需要文案工作流，应从 `docs/use/content.md`、`src/config/single-site*.ts`、当前页面目标和用户提供的业务素材开始。
...
**自动处理：** 扫描 `docs/superpowers/workflows/cwf/{page}/` → 确定版本号 v{N}。
...
**输出**: `docs/superpowers/workflows/cwf/{page}/v{N}.md`
...
```
docs/superpowers/workflows/cwf/{page}/
├── v1.md
├── v{N}-final.md
messages/                # 可选
├── en/{page}.json
...
```
文案定稿完成: docs/superpowers/workflows/cwf/{page}/v{N}-final.md
下一步: /dwf → 读取定稿进入设计流程
```

## 会话恢复

检测到 `docs/superpowers/workflows/cwf/{page}/` 存在 → 显示当前状态 → 询问：继续 / 新版本。
