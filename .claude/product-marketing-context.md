...
| 新项目团队 | 是否能快速换品牌上线 | 内容和配置散落难替换 | `docs/use/`、`docs/ref/`、`src/config/single-site*.ts`、`content/` 和 `messages/` 提供替换面 |
...
4. **Replaceable business surface**：品牌和内容替换面集中在 `docs/use/`、`docs/ref/`、`src/config/single-site*.ts`、`content/`、`messages/`。
...
| “示例内容是不是会污染新项目？” | 示例内容必须在派生项目中替换；`brand:check` 和 `docs/use/replace.md` 用来帮助检查。 |
...
- 组件治理：`docs/design/impeccable/system/COMPONENT-GOVERNANCE.md`
