...
/ai-smell-audit truth   # + truth-source drift 检查，读 docs/use/ docs/ref/ docs/proof/ .claude/rules/ messages/
...
- `audit-report-YYYYMMDD.md` —— 技术证据主报告，结构：
  ...
  - §6 Delete-first repair plan（可删 / 可合并 / 可收口 / 可去除）
  - §7 Phase 3 3-truth 数据流（Runtime / Proof / Design truth）
  - §8-§9 架构心智模型 + 附录（含复核抽检公示 + skill 自检结果）
- `/tmp/audit/*` —— 中间产物
...
- ✅ 报告 §1 是非技术 owner 能读的中文；§5 Change-cost 强制回答"下次改动痛不痛"，§6 Delete-first 强制回答"能不能删"
