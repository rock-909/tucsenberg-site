...
**Standalone** — the user wants an AI-smell read specifically. Run everything
below: Phase 0 preflight through Phase 4 report.

**As a lane inside `repo-health-audit`** — that skill is the authoritative
whole-repo orchestrator for this project and it routes the tests / AI-smell /
dead-code lane here. In that mode, **do not re-run the framework around this
lane**: preflight and scope gating, evidence admissibility rules, business-flow
selection, clustering, the report template, and the owner summary all come from
`repo-health-audit` — running a second copy produces two preflights, two report
shapes, and two sets of evidence rules for one audit.

What this skill contributes in lane mode, and nothing else:

- the AI-smell taxonomy (`references/smell-taxonomy.md`)
- fake-green / test-theater detection — tests that cannot fail, mocks standing
  in for the thing under test
- dead code and ghost code that reads as alive
- locally-consistent / globally-divergent pattern drift
- change-cost and speculative-abstraction diagnosis

Return findings in the orchestrator's Finding Contract shape and stop there.
If it is ambiguous which mode you are in, ask — one question is cheaper than a
duplicated audit.
...
/ai-smell-audit truth                 # + Lane C. Adds docs/use/, docs/ref/, docs/proof/, .claude/rules/, content/, messages/.
...
| truth | `src/**` + `docs/{use,ref,proof}/**` + `.claude/rules/**` + config | + Truth-source drift, shared-layer business leakage   | Proof-lane erosion (unless `--full`)             |
...
Surfaces Lane C reads: `docs/use/**`, `docs/ref/**`, `docs/proof/**`, `.claude/rules/**`, `AGENTS.md`, `CLAUDE.md`, `content/**`, `messages/**`, canonical config (e.g., `src/config/single-site*.ts` for this starter), and every file mentioned as "canonical" by any of the above.
