...
- Cross-check any `docs/{use,ref,proof}/*CONTRACT*.md` proof claims against the current repo tree.
...
>
> Lane C runs only in `truth` or `full` scope modes. Claims it produces are only meaningful if the audit actually read the surfaces listed in the mode's scope matrix (`docs/use/`, `docs/ref/`, `docs/proof/`, `.claude/rules/`, `content/`, `messages/`, config).
...
**Detection**:
- Enumerate every `.claude/rules/*.md` and current truth doc under `docs/use/`, `docs/ref/`, and `docs/proof/`.
