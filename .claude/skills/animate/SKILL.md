...
**For all animation decisions — easing, duration, spring config, gesture/drag, performance, and component animation patterns — the `emil-design-eng` skill is the authoritative source.** Consult it first for animation rules. This skill provides the project-specific workflow (Impeccable integration, assessment checklist, verification) that wraps around Emil's animation principles.

When rules in this file conflict with `emil-design-eng`, Emil's rules take precedence.
