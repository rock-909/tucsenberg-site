...
For this starter, `DESIGN.md` is a project-local design operating model. The file may keep machine-readable frontmatter and stable markdown sections, but the source of truth is the live site, current components, `PRODUCT.md`, `docs/design-truth.md`, and confirmed Impeccable system docs.

## The frontmatter: token schema

The YAML frontmatter is the machine-readable layer. Keep it tight; every entry should correspond to a token the project actually uses.
...
- For starter color work, follow the project color contract first: keep role-based tokens and avoid raw production color values.
...
- **Colors**: Group into Primary / Secondary / Tertiary / Neutral when those roles match the project-local design system. If the project only has one accent, express it as Primary + Neutral — omit Secondary and Tertiary rather than inventing them.
...
From the auto-extracted tokens, draft the YAML frontmatter now (you'll write it at the top of DESIGN.md in Step 4). This is the machine-readable layer used by local design tooling and agents.
...
- **Components**: one entry per variant (`button-primary`, `button-primary-hover`, `button-ghost`). Reference primitives via `{colors.X}`, `{rounded.Y}`. If a variant needs a property the compact component frontmatter does not cover (shadow, focus ring, backdrop-filter), carry the full snippet in the sidecar instead.
...
The frontmatter owns token primitives (colors, typography, rounded, spacing, components). The sidecar at `DESIGN.json` carries **what compact frontmatter should not hold**: tonal ramps per color, shadow/elevation tokens, motion tokens, breakpoints, full component HTML/CSS snippets (the panel renders these into a shadow DOM), and narrative (north star, rules, do's/don'ts). It extends the frontmatter, it doesn't duplicate it.
...
**What changed from schemaVersion 1.** The old sidecar carried token primitive arrays (`tokens.colors[]`, `tokens.typography[]`, etc.). Those values now live in the frontmatter. The sidecar only carries metadata that should not live in compact frontmatter — tonal ramps, canonical OKLCH when the hex is an approximation, display names, role hints — keyed by the frontmatter token name (`colorMeta.<token-name>`, `typographyMeta.<token-name>`). Components still carry full HTML/CSS because the compact component frontmatter cannot hold them.
...
- **Concrete anti-pattern tests**. Use direct checks like *"If it looks like a 2014 app, the shadow is too dark and the blur is too small."* A one-sentence audit test beats a paragraph of principle.
...
- Don't invent top-level frontmatter token groups beyond `colors`, `typography`, `rounded`, `spacing`, and `components` (no `motion:`, `breakpoints:`, `shadows:` at the top level). Anything else belongs in the sidecar's `extensions`.
