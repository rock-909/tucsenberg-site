// Parse a DESIGN.md project design spec into a structured JSON model that
...
// ---------- Frontmatter (compact DESIGN.md YAML subset) ----------
...
// Minimal YAML reader for the DESIGN.md frontmatter subset: scalar maps with
// one level of nested objects (typography roles, components). Indent-based,
// 2-space convention. No arrays, no anchors, no multi-line scalars — accepting
// those would require a real YAML dependency we don't want to vendor.
...
function extractNamedRules(lines) {
  ...
  // Style B: `### The "X" Rule` or `### The X Fallback`, body is the
  ...
  // Style C bullet form: "*   **The Layering Principle:** body"
...
function parseColorBullet(bullet) {
  ...
  // Case 2: **Name (values):** description — value embedded in bold.
  const groupedRoleMatch = text.match(/^\*\*([^*]+?)\s*\(([^)]+)\):\*\*\s*(.*)$/);
  if (groupedRoleMatch) {
    return buildColor(groupedRoleMatch[1].trim(), groupedRoleMatch[2], groupedRoleMatch[3]);
  }
...
function parseRoleInlineGroups(lines) {
  // Legacy grouped format: `*   **Primary (`#00478d` to `#005eb8`):** Use for "..."`
...
function extractTypography(section) {
  ...
  // Pattern B: *   **Display & Headlines (Noto Serif):** description
  if (Object.keys(fonts).length === 0) {
    const groupedRoleRe = /\*\*([\w\s&/]+?)\s*\(([^)]+)\):\*\*\s*(.+)/g;
    let sm;
    while ((sm = groupedRoleRe.exec(text)) !== null) {
      const rawRole = sm[1]
        .trim()
        .toLowerCase()
        .replace(/\s*&\s*/g, '-')
  ...
  // first free paragraph under the section header.
...
function normalizeFontRole(raw) {
  ...
  // Legacy specs may write compound roles like "display-&-headlines" or "ui-&-body"
...
function extractElevation(section) {
  ...
  // Fallback: extract shadows written inline in prose, e.g.
