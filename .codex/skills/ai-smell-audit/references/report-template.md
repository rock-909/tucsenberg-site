...
| §5 | Delete-first repair plan（四固定小节；空则写"无"） | Tech → Owner sign-off | No |
| §6 | Phase 3 数据流深读（3-truth：Runtime / Proof / Design） | Tech | Per-flow |
| §7 | 架构心智模型（Phase 1 产出；跨审计复用） | Tech | Skim OK |
| §8 | 附录（未覆盖说明 / 抽检公示 / skill 自检 / 效果边界） | Tech | No |

Read order suggestion:
- Owner path: `audit-owner-summary-YYYYMMDD.md`
- Tech path: Header → §0 → §2 → §3 + §4 并读 → §5 → §6 → §7-§8.
...
| Tooling drift 检测 | `{skill 三文件自检结果；若 drift 存在，此处链接到 §8.6}` |
...
**推荐收口路径**：{1-2 句。若指向 §5 某项 delete-first，在此引用 `§5.X`}
...
### 5.5 Delete-first 优先级建议

{基于 §5.1-5.4 内容，给出 owner 视角的"本月做哪一条、下季做哪一条"排序。若四节都为无，本节写"本轮审计未发现 delete-first 空间——这本身不坏，但下次审计应挑战此结论"}

---

## 6. Phase 3 数据流深读（3-truth 输出）
...
### 6.1 询盘提交路径（lead submission）

{上述模板的具体填充}

### 6.2 产品 / 目录渲染（product catalog rendering）
...
### 7.2 偏离 `.claude/rules/architecture.md` 的地方

{列出每处声明的规则 vs 实际代码的差异}

### 7.3 本轮审计新识别的 canonical 真相源
...
Lane D 回答"建议删除"的 findings 列表，及其落地到 §5 的映射：
- `F-...` → §5.X

### 8.6 Skill 自检结果（Phase 0.5）
