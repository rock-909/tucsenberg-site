---
name: analyzer
description: Produce a structured multi-dimensional analysis report from any content (URL, article, tweet, transcript, or local file). Scans across core arguments, tools & methods, workflow ideas, data & numbers, risks & warnings, resources, mental-model shifts, and action items grouped by effort (quick wins / deeper work / exploration). Trigger when the user says "/analyze [URL]", "analyze this article", "what are the key takeaways", "structured analysis", or immediately after a video/podcast transcription when the user wants insights. Supports full, sparse (hit dimensions only), and brief (action items only) modes.
---
