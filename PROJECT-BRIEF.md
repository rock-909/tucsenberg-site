...
> **[Step 4.1 supersession]** 本节（含兼容矩阵页 / 产品页等页面类型描述）为历史规划叙事，保留不删。Step 4.1 起：页面 **STRUCTURE** 以 `DESIGN.md §7.1–§7.5` 为准；页面 **FACTS** 以 `src/data/product-compatibility/**` 为准（17 条兼容路径、3 个 OEM 品牌：Sanitaire / EDI / SSI Aeration）。Step 4.1 在此结构内恢复 PROJECT-BRIEF 要求的信任模块（TrademarkDisclaimer / SlaCommitments / CompatibilityProofBox / MaterialDecisionCard / BatchControlsBlock / NarrativeSection），但任何与数据层冲突的品牌或兼容条数计数以数据层为准。
...
> **[Step 4.1 supersession]** 产品页历史叙事保留不删。Step 4.1 起：产品页 **STRUCTURE** 以 `DESIGN.md §7.3` 为准；**FACTS**（兼容品牌列表、规格参数、材质）以 `src/data/product-compatibility/**` 为准（17 条兼容路径、3 个 OEM 品牌：Sanitaire / EDI / SSI Aeration）。原文提到的"阶梯定价信号"等数字承诺不再写死，统一改为"报价时确认"措辞（见 DEVELOPMENT-LOG「Step 4.1 内容处置」）。Step 4.1 恢复 MaterialDecisionCard / BatchControlsBlock 等信任模块，但事实源自数据层。
...
> **[Step 4.1 supersession]** RFQ / 报价页（及相关首页入口、URL 结构）历史叙事保留不删。Step 4.1 起：报价页与首页 **STRUCTURE** 以 `DESIGN.md §7.4`（报价页）/ `DESIGN.md §7.1`（首屏）/ `DESIGN.md §7.5`（跨页交互）为准；**FACTS** 以 `src/data/product-compatibility/**` 为准（17 条兼容路径、3 个 OEM 品牌：Sanitaire / EDI / SSI Aeration）。交期/MOQ/年限等数字档统一改为"报价时确认"，第二询价信箱收敛为单一 `sales@tucsenberg.com`（见 DEVELOPMENT-LOG「Step 4.1 内容处置」）。Step 4.1 恢复 SlaCommitments / CompatibilityProofBox 等信任模块，事实源自数据层。
