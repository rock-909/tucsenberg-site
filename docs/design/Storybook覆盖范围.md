...
**范围**：现有业务组件目录，例如 `src/components/forms/`、`src/components/products/`、`src/components/footer/`、`src/components/navigation/`。不要引用已删除的目录（例如旧 `trust/` 或已退役的 `src/components/contact/`）。
