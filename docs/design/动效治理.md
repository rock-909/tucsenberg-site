...
- The former `--ease-spring` overshoot token was retuned and renamed to
  `--ease-out-quint` (2026-07-07); no bounce/elastic easing token exists anymore.
...
`motion/react` is a route-scoped advanced capability on this site, not a global
layout dependency. The homepage owns `LazyMotion`, `BreathingReveal`, and the
light-breathing reveal tokens. The root locale layout and non-home routes must
not import `motion/react`, `LightMotionProvider`, or `BreathingReveal`. Site-wide
route enter motion stays in the CSS `PageTransition` wrapper.
...
- Hero and above-the-fold claim content must be server-rendered and visible by
  default.
- Homepage section reveal is the only approved `motion/react` consumer. Keep the
  runtime in `src/app/[locale]/page.tsx`, not the root layout or other routes.
