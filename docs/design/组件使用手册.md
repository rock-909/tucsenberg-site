...
judgment lives in `.claude/rules/ui.md`; the machine-readable inventory —
every wrapper with its layer, surface, client boundary, and use/avoid rules —
is `src/components/component-governance.registry.json`.
...
The registry is the inventory; this file is the shortlist. The Markdown mirror
of the registry that used to sit beside this file was retired 2026-07-26 — it
restated all 24 entries field for field, and the test that kept the two in sync
only ever fired when someone edited one copy.
...
| Checkbox | `Checkbox` | Governed boolean choices; do not migrate cookie consent or other no-JS critical consent without separate proof. |
...
- Contact and Request Quote share the same `InquiryForm`, `Input`, `Textarea`, `Card`, and static fallback; there is no privacy consent checkbox on either page.
...
Storybook preview loads composed physical-pack messages for component
states only; those messages are not a second runtime authoring surface.
...
Inquiry form panels reuse `Card`; static fallback panels use the same wrapper.
...
Inquiry form fields reuse `Input` and `Textarea`. Inquiry form panels reuse
`Card`. No inquiry-only input, textarea, or card wrapper is needed.
...
2. Storybook story;
3. focused test;
4. component governance registry entry (the registry schema is enforced — a
   missing or malformed field fails the governance scan, so no field list needs
   restating here);
5. a row in the "Choose components" table above, if the wrapper is one an agent
   would reach for by name;
6. `pnpm component:governance` proof.
