...
If Storybook reports `unable to find package.json for @opennextjs/cloudflare`, treat it as dependency metadata noise unless it also reproduces in `pnpm build` or `pnpm website:build:cf`. It is not production bundle performance proof.
