...
- 仓库默认 owner 目前是 single-owner；不存在可强制的 backup repository owner
...
5. 如果 backup review path 当前不可用，就要么缩 scope，要么明确这是 single-owner 风险，不能假装仓库已经 diversified
...
- 当前还存在的硬约束：
  - 实际维护能力已经不止一个人
  - 还没有第二个 enforceable repository owner identity 真正独立分担 Tier A 吞吐
  - 当前不能把历史 backup path 当成有效 owner
