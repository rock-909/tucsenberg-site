...
本次修复当时这条只覆盖了 `/contact`：`/request-quote` 的表单挂在一层
`<Suspense>` 后面，无脚本时整个子树被写到 `<main>` 外的隐藏容器里，定位不到预留
容器。那层边界已于 2026-07-25 拆掉，两条路由现在都在覆盖内——
见 `docs/技术难题/无谓的流式边界让正文落到主内容区之外.md`。
