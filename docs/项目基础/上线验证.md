...
```bash
NODE_ENV=production APP_ENV=preview NEXT_PUBLIC_SITE_URL=https://tucsenberg-site-preview.faints-pudgier-9r.workers.dev NEXT_PUBLIC_BASE_URL=https://tucsenberg-site-preview.faints-pudgier-9r.workers.dev NEXT_PUBLIC_TURNSTILE_SITE_KEY=0x4AAAAAADvC8KDbJ54402pb pnpm build
NODE_ENV=production APP_ENV=preview NEXT_PUBLIC_SITE_URL=https://tucsenberg-site-preview.faints-pudgier-9r.workers.dev NEXT_PUBLIC_BASE_URL=https://tucsenberg-site-preview.faints-pudgier-9r.workers.dev NEXT_PUBLIC_TURNSTILE_SITE_KEY=0x4AAAAAADvC8KDbJ54402pb pnpm website:build:cf
NODE_ENV=production APP_ENV=preview NEXT_PUBLIC_SITE_URL=https://tucsenberg-site-preview.faints-pudgier-9r.workers.dev NEXT_PUBLIC_BASE_URL=https://tucsenberg-site-preview.faints-pudgier-9r.workers.dev NEXT_PUBLIC_TURNSTILE_SITE_KEY=0x4AAAAAADvC8KDbJ54402pb pnpm exec wrangler deploy --dry-run --env preview
```
...
- Page copy and product specifications match the owner source files under
  `/Users/Data/workspace/tucsenberg/内容/文案/`, except the current caveats below.
  In this repo, current product detail copy,
...
Current owner caveats carried into launch review:

- The ABS product source once used a price-number headline. The public site uses
  `Not the disposable kit you saw on Amazon` to keep the no-price rule intact;
  owner should approve the final headline before public launch.
- `TB-TD500` section weight is published as `Available on request`. Replace it
  only when owner provides a verified value.
- Public PDF remediation, public phone/contact decisions, product photos,
  legal/contact signoff, analytics/privacy signoff, and recipient-chain proof
  remain owner-side launch inputs.
...
- Do not introduce `src/proxy.ts` just to silence framework warning.
