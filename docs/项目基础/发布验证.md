...
- `cf-static-asset-headers` reads `public/_headers` and
  ...
  rule overrides. Those are the two protected subtrees; other published assets
  (`favicon.ico`, `fonts/`, `images/`, `.well-known/`) carry no policy this check
  enforces, so it does not compute their URLs. It does not require any particular
