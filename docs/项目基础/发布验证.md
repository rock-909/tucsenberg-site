...
- `cf-static-asset-headers` reads `public/_headers` and
  `.open-next/assets/_headers`, works out the URL every published file is
  actually served from, and applies wrangler 4.100.0's own rule semantics to
  prove what each file really receives: every download carries
  `X-Robots-Tag: noindex`, and every `_next/static` file carries a one-year
  immutable `Cache-Control` that no later rule overrides. It does not require
  any particular route to be written — a different spelling that still covers
  every real file passes. It also refuses to pass, rather than guess, when
  something makes the served URL unknowable: a duplicated route, a host that is
  not one exact literal, a header name the runtime rejects, a directory whose
  on-disk name differs from the one in the URL, a `.html` download, an empty
  protected directory, a directory or entry it could not read, a `_redirects` or
  `.assetsignore` file, or a `wrangler.jsonc` with no `assets.directory`. It runs
  on every pull request in `ci.yml` as well as in `release:verify`.
