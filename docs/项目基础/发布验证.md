...
```bash
gh workflow run airtable-phone-proof.yml --ref <branch-or-tag-containing-target-sha> -f airtable_table_name=Contacts
gh run watch --exit-status
```

Dispatch `--ref` must be a branch or tag that points at the commit you intend to prove; do not assume a raw commit SHA works as a dispatch ref until you have verified it in this repo. After the run finishes, confirm the workflow run `headSha` equals the exact commit SHA of the PR you intend to prove. A green default CI run on that SHA is not the same as a green phone proof run.
