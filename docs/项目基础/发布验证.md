...
- `cf-static-asset-headers` reads `public/_headers` and
  ...
  protected directory, a `_redirects` or `.assetsignore` file, or a
  `wrangler.jsonc` with no `assets.directory`. It runs on every pull request in
  `ci.yml` as well as in `release:verify`.
