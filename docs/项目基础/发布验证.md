...
- The local Playwright step rebuilds `.next`, so it runs before the Cloudflare
  artifact proof.
- `cloudflare-artifact-config` proves the generated Next and OpenNext
  `required-server-files.json` configs keep the Cloudflare-safe values.
- `cf-static-asset-headers` proves OpenNext copied `public/_headers` into
  ...
  and the `X-Robots-Tag: noindex` rule for `/downloads/*`.
- Warnings that appear only inside generated `.open-next/**` artifacts, such
  as `duplicate-case`, `direct-eval`, or `equals-negative-zero`, are generated
  bundle warnings. Do not claim a source edit fixed them unless a fresh
  `pnpm website:build:cf` and Wrangler dry-run prove they are gone.
...
- Release runbooks and manifest tests should lock command intent and current
  sequence, not point-in-time outputs such as commit hashes, warning counts, or
  push status.
