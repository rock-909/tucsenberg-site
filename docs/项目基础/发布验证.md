...
- `cf-static-asset-headers` reads `public/_headers` and
  ...
  document is the file's extension (`.pdf`, `.docx`, `.xlsx`, `.csv`, `.txt`,
  `.zip` and the like), not the folder it is in; anything whose extension is not
  on that list — images, fonts, JavaScript — carries no crawl policy this check
  enforces, and neither do the files crawlers are supposed to read
  (`.well-known/`, `robots.txt`, `ads.txt`). It does not require any particular
