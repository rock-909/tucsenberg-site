...
- `cf-static-asset-headers` reads `public/_headers` and
  ...
  which of the two wins cannot be proven, an `assets` setting it does not model (such as
  `html_handling` or `run_worker_first`), or a `wrangler.jsonc` with no
  `assets.directory`. It runs on every pull request in `ci.yml` as well as in
  `release:verify`.
