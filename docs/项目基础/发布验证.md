...
- `cf-static-asset-headers` reads `public/_headers` and
  ...
  Exactly three full paths are let through, each because its own standard fixes
  it there: `/robots.txt`, `/ads.txt` and `/.well-known/security.txt`. Anything
  else in those folders — a `.pdf` or a `.txt` price list dropped into
  `.well-known/` — is still demanded to carry `noindex`. It does not require any particular
