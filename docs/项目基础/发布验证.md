...
- `cf-static-asset-headers` reads `public/_headers` and
  ...
  rule overrides and no `Vary: *` makes unreusable. It also sweeps the whole
  published tree for document files (`.pdf`, `.docx`, `.xlsx`, and the like)
  sitting outside `downloads/` and demands `noindex` on those too, wherever they
  are: a quotation dropped into `public/` by mistake, or a PDF the build moved
  into `_next/static/media/`, would otherwise be published with no rule covering
  it at all. Other published assets (images, fonts, `.well-known/`) carry no
  policy this check enforces. It does not require any particular route to be
  written — a different spelling that still covers every real file passes. It also refuses to pass, rather than guess, when something makes the
  ...
  (such as `html_handling` or `run_worker_first`), or a `wrangler.jsonc` with no
