# Brand

Brand facts live in config and content, not page components.

## Edit first

- `src/config/single-site.ts`: company identity, contact, social links, default SEO.
- `src/config/single-site-seo.ts`: sitemap, robots, crawl/index truth.
- `src/config/single-site-navigation.ts`: navigation labels and active links.
- `src/config/single-site-links.ts`: shared hrefs.
- `src/config/single-site-page-expression.ts`: homepage/page expression.

For product-facing brand claims, also review:

- `src/config/single-site-product-catalog.ts`
- `src/constants/tucsenberg-product-page-*.ts`
- `src/constants/tucsenberg-product-pages.ts`

## Then edit

- `content/pages/{locale}/*.mdx`
- `messages/base/**`
- `messages/profiles/b2b-lead/**`
- `messages/profiles/catalog/**`
- `public/images/**`

## Public images

Every tracked file under `public/images/**` must have a live production
reference from config, content, messages, or source. The current image set is:

| File | Owner | Used for |
| --- | --- | --- |
| `public/images/tucsenberg-logo.png` | `src/config/single-site.ts` -> `brandAssets.logo.horizontal` | Header and structured-data logo |
| `public/images/tucsenberg-logo-square.png` | `src/config/single-site.ts` -> `brandAssets.logo.square` | Square/avatar logo slot |
| `public/images/tucsenberg-og.png` | `src/config/single-site.ts` -> `brandAssets.ogImage` | Default Open Graph / social preview |

When adding a new image, add the file and wire its `/images/...` path into the
production owner in the same change. Page-level `seo.ogImage` paths must point
to real files; `pnpm content:check --strict-frontmatter` reports
`missing_og_image` when they do not.

## Do not

- Do not hardcode company facts in components.
- Do not add a second site-definition mirror.
- Do not treat starter placeholder facts as real client claims.

## Navigation breakpoints

```text
< 840px       mobile menu
840-1199px   compact desktop nav
>= 1200px    full desktop nav
```

The rule is not device name. The rule is: show main navigation as early as possible without crowding brand, language switcher, or CTA.
