...
| product catalog | profile-gated | Current Tucsenberg product-page runtime truth: `src/constants/tucsenberg-product-pages.ts`. Catalog route/list adapters: `src/config/single-site-product-catalog.ts`, `src/constants/product-standards.ts`. Legacy starter/adopter fixture: `src/constants/product-specs/**` | catalog tests when `catalog` is selected; product page contract tests for current Tucsenberg pages |
...
Catalog compatibility note: Current Tucsenberg product detail pages and their
JSON-LD use `src/constants/tucsenberg-product-pages.ts` as runtime truth.
`src/config/single-site-product-catalog.ts` and
`src/constants/product-standards.ts` still provide route/list/catalog adapters.
`src/constants/product-specs/**` is a legacy starter/adopter fixture unless a
future approved profile explicitly selects that path. In this source repo, the
legacy starter demo data may re-export from
