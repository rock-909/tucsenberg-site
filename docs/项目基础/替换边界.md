...
| UI messages | `review-or-tune` | `messages/base/**`, `messages/profiles/b2b-lead/**`, `messages/profiles/catalog/**`, compat `messages/{locale}/critical.json`, `messages/{locale}/deferred.json` | `pnpm messages:sync && pnpm content:check` |
| email copy | `must-replace` | `messages/base/{locale}/deferred.json` `emailTemplates`, `src/emails/email-copy.ts`, `src/lib/email/runtime-email-content.ts`, `src/lib/resend-utils.ts` | `pnpm exec vitest run tests/architecture/email-copy-boundary.test.ts tests/architecture/email-runtime-boundary.test.ts src/emails/__tests__/email-copy-source.test.ts src/emails/__tests__/theme.test.ts src/lib/__tests__/resend.test.ts` |
