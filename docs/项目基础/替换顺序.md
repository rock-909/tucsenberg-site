...
`src/constants/tucsenberg-product-pages.ts` and
`src/config/single-site-product-catalog.ts`. Current Tucsenberg product detail copy,
...
browser form submission on `/api/inquiry` unless the lead-flow contract is
redesigned with matching route, form, and security tests. `/api/contact` is a
temporary non-writing tombstone owned for final removal by D6e.
