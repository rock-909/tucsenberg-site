...
当前 Tucsenberg 派生站使用 `catalog` 作为公开产品面，`/products/[market]`
是当前公开产品详情页。旧 starter 的 `company-site` 说明只作为 adopter/profile
背景，不覆盖当前站点事实。
...
is retained as legacy starter/adopter compatibility material, not the current
product truth for this derived site.
