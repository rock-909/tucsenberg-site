...
> Historical. The multi-profile / starter-factory materialize flow described here
> has been retired. The site is permanently the single English `catalog` site,
> and the `scripts/starter-profile` materialize tooling and profile fixtures no
> longer exist. This page is kept only as a historical record.
