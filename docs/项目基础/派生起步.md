...
> Historical. The multi-profile / starter-factory derivation flow described here
> has been retired. The site is permanently the single English `catalog` site,
> and the `scripts/starter-profile` materialize tooling no longer exists. This
> page is kept only as background on how the site was derived.
...
1. Replace real company content: `替换顺序.md`.
2. Check profile boundary: `派生配置.md`.
3. Run launch proof: `上线验证.md`.
