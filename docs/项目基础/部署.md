...
- production strict gate 当前要求 Upstash Redis；不要把 KV-only 当作生产替代方案。`KV_REST_API_URL` / `KV_REST_API_TOKEN` 仅保留在 env schema 与 production-config 校验中，用于拒绝 KV-only 误配，不是活跃限流后端。
...
| security / rate limit | `RATE_LIMIT_PEPPER`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` |
...
```bash
NEXT_PUBLIC_TURNSTILE_SITE_KEY=<public-site-key> pnpm build
NEXT_PUBLIC_TURNSTILE_SITE_KEY=<public-site-key> pnpm website:build:cf
NEXT_PUBLIC_TURNSTILE_SITE_KEY=<public-site-key> pnpm exec wrangler deploy --dry-run --env preview
```
