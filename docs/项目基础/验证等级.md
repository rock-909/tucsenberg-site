...
| `pnpm website:lighthouse` | manual performance proof | current built page performance lab checks (runs its own production build) | default CI, git hook, or release approval |
...
新增或修改 gate 时，至少留一条窄证明：把坏形状放进去会变红，撤回后重新变绿。
不能因为当前仓库刚好通过，就把这个 gate 算作有效。
...
- browser proof: every spec under `tests/e2e/`
- real service canary: `tests/e2e/smoke/` against the deployed URL

Playwright discovery is not filtered anywhere: `testDir` decides, so a new
`tests/e2e/*.spec.ts` runs in the PR lane, the nightly cross-browser lane, and
the local release proof without being added to a list. The three lists that used
to name individual files disagreed with each other, and three spec files ended up
in none of them.
