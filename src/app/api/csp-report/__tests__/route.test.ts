...
async function callOPTIONS(request: NextRequest) {
...
describe("CSP Report API Route", () => {
  ...
  describe("OPTIONS /api/csp-report", () => {
    it("应该为允许的来源返回共享CORS headers", async () => {
      const response = await callOPTIONS(createOptionsRequest());
    ...
    it("应该拒绝不在允许列表中的预检来源", async () => {
      const response = await callOPTIONS(
        createOptionsRequest("https://evil.test"),
      );
