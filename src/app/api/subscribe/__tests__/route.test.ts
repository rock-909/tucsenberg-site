...
describe("/api/subscribe route", () => {
  ...
  it("treats whitespace-only email as invalid, not missing", async () => {
    const response = await POST(
      makeSubscribeRequest({
        email: "   ",
        turnstileToken: "valid-token",
  ...
  it("maps processLead validation failures to the subscribe validation error", async () => {
  ...
  it("maps processLead processing failures to the subscribe processing error", async () => {
    vi.mocked(processLead).mockResolvedValueOnce({
      success: false,
