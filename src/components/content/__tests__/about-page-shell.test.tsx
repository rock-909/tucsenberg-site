...
describe("AboutPageShell", () => {
  ...
  it("keeps demo stat numbers out of the hero so they do not read as claim proof", () => {
    render(
      <AboutPageShell
        metadata={baseMetadata}
        content="## Body"
    ...
    const heroClaim = screen.getByRole("heading", {
      level: 1,
      name: "A showcase website starter designed for real public launch preparation",
    });
    const statProof = screen.getByText("Starter baseline");

    // The stat labels still render in their own section on the page.
    expect(statProof).toBeInTheDocument();
    expect(screen.getByText("Locales")).toBeInTheDocument();
    expect(screen.getByText("Replacement surfaces")).toBeInTheDocument();
    expect(screen.getByText("Launch path")).toBeInTheDocument();

    // But the placeholder demo stats must NOT sit inside the hero block next to
    // the main claim, where starter demo numbers would read as real proof.
    const heroSection = heroClaim.closest("section");
    expect(heroSection).not.toBeNull();
    expect(heroSection).not.toContainElement(statProof);

    // The hero claim and the products CTA structure both stay intact.
  ...
  it("renders CTA with link to products for the default company-site profile", () => {
    render(
      <AboutPageShell
        metadata={baseMetadata}
        content="## Body"
