...
const STANDARD_STATUS_BADGE_VARIANTS = {
  certified: "success",
  applying: "warning",
  compliant: "secondary",
} satisfies Record<
...
function CommitmentList({ items }: { items: QualityCommitmentItem[] }) {
  return (
    <div className="grid grid-cols-1 gap-px overflow-hidden rounded-[var(--card-radius)] border border-border bg-border shadow-[var(--shadow-card)] lg:grid-cols-2">
      {items.map((item) => {
        const Icon = COMMITMENT_ICONS[item.key] ?? StaticLayoutGridIcon;
        return (
          <div
            key={item.key}
            className="flex items-start gap-4 bg-card px-6 py-5 transition-colors duration-150 hover:bg-muted"
          >
    ...
    <div className="mt-4 flex flex-wrap items-center gap-3">
      {content.standards.map((standard) => (
        ...
        >
          ...
          <Badge
            variant={STANDARD_STATUS_BADGE_VARIANTS[standard.status]}
            className="text-[11px]"
          >
