...
describe("mdx-loader manifest-only runtime behavior", () => {
  beforeEach(() => {
    vi.resetModules();
    resolveOptionalContentEntryMock.mockReset();
  ...
  it("returns null when the manifest entry is missing", async () => {
    resolveOptionalContentEntryMock.mockReturnValue(undefined);
  ...
  it("returns null when the manifest entry exists but no importer exists", async () => {
    resolveOptionalContentEntryMock.mockReturnValue({
      type: "pages",
      locale: "en",
