  ...
  describe("性能和内存测试", () => {
    ...
    it("should not create excessive memory usage with repeated calls (smoke check)", async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        lastSchema = await generateLocalizedStructuredData(
          "en",
          "Organization",
          {},
        );
        lastJsonLd = generateJSONLD({ test: "data" });
      }

      expect(lastSchema).toMatchObject({
        "@context": "https://schema.org",
        "@type": "Organization",
        name: "Example Showcase Company",
      });
      expect(JSON.parse(lastJsonLd)).toStrictEqual({ test: "data" });
    });
  });
