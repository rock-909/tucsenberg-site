...
interface SeoPageCase {
  path: string;
  name: string;
  expectedGraphTypes: string[];
  expectedArticleDateModified?: string;
}

const KEY_PAGES: SeoPageCase[] = [
  {
    path: "/en",
    name: "Homepage",
  ...
  {
    path: "/en/blog/prepare-before-launch",
    name: "Blog article",
    expectedGraphTypes: [
      "Organization",
      "WebSite",
    ...
    expectedArticleDateModified: "2026-05-05",
  },
];
...
for (const {
  ...
  test.describe(`SEO: ${name} (${path})`, () => {
    ...
    test("has JSON-LD structured data", async ({ page }) => {
      ...
      if (expectedArticleDateModified) {
        const articleNode = parsed["@graph"].find(
          (node: Record<string, unknown>) =>
            schemaTypes(node["@type"]).includes("Article"),
        );

        expect(articleNode).toBeTruthy();
        expect(articleNode.dateModified).toBe(expectedArticleDateModified);
      }
    });
